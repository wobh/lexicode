...
- as values in form fields
- as columns in a table row
- as "key: value" pairs in a record (listitems, generally)
- as "x-www-form-urlencoded" form submission
- as JSON or JSON-API "application/vnd.api+json"
- as XML

But in most all of these cases, none of it requires the application to
do anything with directly (validation and normalization being
exceptions I don't want to dismiss, but don't want to get distracted
with either).

Mostly, we're talking about browser application whose concerns are
oriented around displaying and updating the display, revealing or
...
with role-like names as prefixes, eg. ~listPosts~.
